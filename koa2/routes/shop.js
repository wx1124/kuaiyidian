...
router.get('/kitchen',async function (ctx, next) {
	let loginbean = ctx.session.loginbean;
  	if(loginbean.shoprole==2){
  		let sql = 'select od.dishname,t.tablenum,od.num,od.remark,od.updtime from orderdishes od,tables t where od.shopid=? and od.state=1 and od.tableid=t.id order by updtime';
  		let dishList = await sequelize.query(sql,{replacements: [loginbean.shopid]});
  		ctx.body = dishList;
  	}
})
